...
EMTboost_HE_real <- function(train.dat, 
                        test.dat, 
                        rho=1.5, 
                        maxsteps=100, 
                        ...
                        cv.folds = 5)
{
  ...
  ans <- optimize(f=dtweedie.nlogl.weight, maximum=FALSE, interval=c(low.limit, high.limit),
                  power=rho, mu=pred_mu, y=Y, weight=1)#as.numeric(Y!=0) )
  phi <- ans$minimum
...
EMTboost.profile_real <- function(train.dat, 
                                  test.dat,
                                  rho.list,
                                  maxsteps=100,
                                  ...
                                  cv.folds = 5)
{
  ...
  for (m in 1:num.rho){
    rho <- rho.list[m]
    print(paste("rho:",rho))
    EMTboost <- EMTboost_HE_real(train.dat=train.dat, 
                            test.dat=test.dat, 
                            rho=rho, 
                            maxsteps=maxsteps,
                            verbose=verbose, 
